...
@section('content')
    <div class="animate-fade-up">
        ...
        <h1 class="text-2xl font-bold text-slate-800 mb-6">Edit Task
</h1>

        <form action="{{ route('tasks.update', $task) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-shadow">{{ old('description', $task->description) }}</textarea>
            </div>
